...
public class TestApp {
	...
	public static void main(String[] args) throws Exception {
		...
		IAuthenticationService authService = authEndpoint.getBasicHttpBindingIAuthenticationService();
		AuthenticationCredential credential = authService.authenticate("Admin1", "Password1");
		String securityToken = credential.getSecurityToken().getValue();
		AdministrationService endpoint = new AdministrationService();
		IAdministrationService  service = endpoint.getBasicHttpBindingIAdministrationService();
		Integer count = service.getAdministratorsCount(securityToken);
		System.out.println(count);
		ArrayOfAdministrator arrayOfAdministrator = service.getAllAdministrators(1, 10, securityToken);
