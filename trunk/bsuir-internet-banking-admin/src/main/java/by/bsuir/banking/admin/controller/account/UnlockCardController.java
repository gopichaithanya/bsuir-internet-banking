...
import by.bsuir.banking.proxy.internetbanking.IInternetBankingServiceUnlockCardDomainFaultFaultFaultMessage;


@Controller
@RequestMapping("/account/{clientId}/card/{accountId}/unlock/{cardId}")
...
public class UnlockCardController extends EntityController {
	...
	public String createForm(HttpSession session, @PathVariable("clientId") Integer clientId,
		...
		try {
			...
			if(!wrapper.isLocked()){
				attrs.addFlashAttribute("error", "Card is not locked");
				return "redirect:/account/"+clientId+"/view/"+accountId;
			}
	...
	public String submitForm(HttpSession session, @PathVariable("clientId") Integer clientId,
		...
		try {
			...
			service.unlockCard(wrapper.getCardNumber(), secretWord.getSecretWord());
		} catch (IInternetBankingServiceGetCardForOperatorAuthorizationFaultFaultFaultMessage e) {
			return "redirect:" + MessageConstants.AUTH_FAILED_VIEW;
		} catch (IInternetBankingServiceGetCardForOperatorDomainFaultFaultFaultMessage e) {
			return "redirect:" + MessageConstants.ERROR_VIEW;
		} catch (IInternetBankingServiceUnlockCardDomainFaultFaultFaultMessage e) {
			return "redirect:" + MessageConstants.ERROR_VIEW;
		}
